...
3. For EVERY answer fill "present" with the concrete concepts/methods demonstrated (or [] if none). Prefer role-relevant concepts from the rubric (e.g. query lifecycle, audit trail, reconciliation, CAPA) — not vague soft skills.
4. "note" must be 2–4 sentences a recruiter can act on: (a) quote or paraphrase the decisive phrase, (b) say why that makes OWNED/SURFACE/EVASIVE/AI, (c) name what is missing if not OWNED. Do NOT write one-word notes like "good" or "thin".
...
Return this exact JSON shape (fill every field; arrays may be empty but must be present):
{
  ...
  "answerGrades": [{ "question": "", "answer": "verbatim answer", "verdict": "AI|OWNED|SURFACE|EVASIVE", "present": ["specific concepts or methods demonstrated"], "note": "2-4 sentences: quote the decisive phrase, explain the verdict, name what is missing if not OWNED — never a comparison to other candidates", "kind": "screen|intent" }],
