...
- Unsupported expertise plus strong evidence the answer was materially generated by AI is a material authenticity/integrity concern. Do not credit the capability, and put the live verification probe in digIn.resolve.
...
4. Keep answerProvenance, authorshipConfidence, and candidateEvidenceCredit independent of each other and of the verdict. The UI must be able to say: "Excellent answer. Career history does not show where they learned this." or "Excellent answer and directly supported by years doing this work."
...
/**
 * Everything about the SEAT rather than the candidate: the rubric, the dimension
 * weights, the learned calibration, the alternate-seat rubrics, and the output
 * schema. This block is byte-identical for every candidate in a job, so it lives
 * in the cached system prefix rather than the per-candidate user message — a batch
 * of N candidates pays for it once at 1.25x and then reads it at 0.1x, instead of
 * N times at full input price.
 *
 * Anything candidate-specific MUST stay out of here or the cache never hits.
 */
function buildSeatBrief(input: EvaluatorInput): string {
  return `SEAT BRIEF — the seat, its rubric, and the output contract. Identical for every candidate on this seat.

SEAT: ${input.seat.jobTitle} — required stratum band ${input.seat.seatStratum}.
SEAT DESCRIPTION:
...
Return this exact JSON shape (fill every field; arrays may be empty but must be present):
{
  ...
  "aiLikelihood": 0.0,
  "roReads": [{ "role": "", "company": "", "years": 0, "stratum": "IIIa", "stratumRange": "IIIa–IVc", "verbs": {"I":[],"II":[],"III":[]}, "read": "one-sentence read of what level this role demonstrates and what burden it maps to", "level": "IIa–III" }],
...
Do NOT echo the candidate's answers, résumé lines, or any other supplied text back verbatim except in the short "quote" fields above — we already hold the source text and match it back by key.`;
}

/**
 * Every field is capped. Interview transcripts and recruiter comment threads grow
 * without bound in Supabase, and an uncapped splice of both is how a single
 * candidate quietly becomes a six-figure-token request. The caps are generous
 * enough that a normal file is untouched.
 */
const LIMITS = {
  resume: 12000,
  answers: 12000,
  coverLetter: 3000,
  publicProfile: 4000,
  interviewEvidence: 24000,
  recruiterComments: 4000,
} as const;

/** Only the candidate. Everything seat-level lives in the cached prefix above. */
...
RÉSUMÉ TEXT:
"""
${input.resumeText.slice(0, LIMITS.resume)}
"""

APPLICATION ANSWERS:
${JSON.stringify(input.answers, null, 2).slice(0, LIMITS.answers)}

COVER LETTER:
${input.coverLetter?.slice(0, LIMITS.coverLetter) || "None"}

PUBLIC PROFESSIONAL PROFILE (for verification only; may be empty):
${input.publicProfile?.slice(0, LIMITS.publicProfile) || "Not fetched — mark profile-dependent claims UNVERIFIABLE."}

INTERVIEW / ASYNC VIDEO EVIDENCE (post-application — weight heavily when present):
${input.interviewEvidence?.trim().slice(0, LIMITS.interviewEvidence) || "None yet"}

RECRUITER NOTES FROM WORKABLE (context only, not primary evidence):
${input.recruiterComments?.trim().slice(0, LIMITS.recruiterComments) || "None"}

Evaluate this candidate against the SEAT BRIEF now and return the JSON shape it specifies.`;
}
  ...
  return values
    ...
    .map((v): AnswerGradePayload => {
      ...
      return {
        ...
        // Only kept if the model volunteered it. The text is joined from
        // applications.answers by question key at read time, so persisting an
        // empty string here would shadow that join.
        ...(typeof v.answer === "string" && v.answer.trim() ? { answer: v.answer } : {}),
        verdict,
        answerQuality,
        answerProvenance,
