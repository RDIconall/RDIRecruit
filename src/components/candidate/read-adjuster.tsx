  ...
  function onSubmit() {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        ...
        setResult(
          (res.overridden ? `Pinned your read on ${candidateName}. ` : res.reanalyzed ? `Re-analyzed ${candidateName} with Claude. ` : "") +
            (res.lesson
    ...
    <div className="mt-4 rounded-xl border border-navy/12 bg-white p-4">
      ...
      {open ? (
        <div className="mt-4 space-y-4">
          ...
          >
            {pending ? "Saving & teaching…" : "Save, teach Claude & re-analyze"}
          </button>
        </div>
      ) : null}
